...
describe.skip('generation verifies the base profile it is about to patch', () => {
  it('generates when the local file is byte-for-byte the recorded base', async () => {
    const response = (await invoke(
      'calibration:generateOrcaProfile',
      generateRequest(),
    )) as { status: string };
  ...
  it('does not stamp generated bytes with an epoch that changed mid-generation', async () => {
    ...
    await invoke('calibration:getOrchestrationStatus', {
      profileId: PROFILE_ID,
...
// Skipped under #756: the saga's CalibrationInstallOrcaProfile channel was removed with the printer-calibration saga in this PR.
describe.skip('generated bytes stay bound to what produced them', () => {
  ...
  onWindows(
    ...
    async () => {
      ...
      const response = (await Promise.resolve(
        registered.get('calibration:installOrcaProfile')?.(undefined, {
  ...
  onWindows(
    ...
    async () => {
      ...
      const response = (await invoke('calibration:installOrcaProfile', {
        profileId: PROFILE_ID,
        projectId: PROJECT_ID,
        snapshotId: SNAPSHOT_SHA,
        operationId: OPERATION_ID,
...
// Skipped under #756: the saga's CalibrationInstallOrcaProfile channel was removed with the printer-calibration saga in this PR.
describe.skip('the pre-write epoch fence stands on its own', () => {
  const onWindows = process.platform === 'win32' ? it : it.skip;
  ...
  function install(profileJsonHash: string): Promise<unknown> {
    return invoke('calibration:installOrcaProfile', {
      profileId: PROFILE_ID,
      projectId: PROJECT_ID,
      snapshotId: SNAPSHOT_SHA,
      operationId: OPERATION_ID,
