...
function fakeHttp(
  ...
  return {
    ...
    startGeneration: vi.fn().mockResolvedValue({ generationJobId: 'gen-123' }),
...
describe('Photo staging behavior', () => {
...
describe('Calibration IPC schema privilege denial', () => {
  it('CalibrationSaveDraft rejects extra renderer-supplied fields', () => {
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationSaveDraft].request.parse({
        profileId: PROFILE_ID,
        projectId: PROJECT_ID,
        operationId: OP_ID,
        fields: {},
        // Extra field not in the schema
        serverUrl: 'http://attacker.com',
      }),
    ).toThrow();
  });

  it('CalibrationStagePhoto rejects arbitrary path fields', () => {
    expect(() =>
  ...
  it('CalibrationGetProject request requires valid UUIDs', () => {
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationGetProject].request.parse({
  ...
  it('CalibrationResolveConflict only accepts semantically valid resolutions', () => {
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationResolveConflict].request.parse({
        profileId: PROFILE_ID,
        conflictId: OP_ID,
        resolution: 'lastWriteWins', // Not a valid strategy
      }),
    ).toThrow();
    // Valid strategies must parse cleanly
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationResolveConflict].request.parse({
        profileId: PROFILE_ID,
        conflictId: OP_ID,
  ...
  it('CalibrationSyncNow request validates profileId is valid UUID', () => {
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationSyncNow].request.parse({
        profileId: 'not-a-uuid',
      }),
    ).toThrow();
    // Valid sync request should parse (projectId is optional)
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationSyncNow].request.parse({
        profileId: PROFILE_ID,
      }),
    ).not.toThrow();
    // projectId as undefined (omitted) is valid
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationSyncNow].request.parse({
        profileId: PROFILE_ID,
        projectId: PROJECT_ID,
  ...
  it('CalibrationStartGeneration requires operationId and baseRevision', () => {
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationStartGeneration].request.parse({
        profileId: PROFILE_ID,
        projectId: PROJECT_ID,
        // Missing operationId and baseRevision
      }),
    ).toThrow();
  });

  it('CalibrationImportLegacyBackupV4 uses approvalId, not arbitrary path', () => {
    expect(() =>
  ...
  it('CalibrationAvailability response validates unavailableReason enum', () => {
    // Valid
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationGetAvailability].response.parse({
        available: false,
        unavailableReason: 'noProfile',
        unavailableDetail: 'No profile selected.',
    ...
    expect(() =>
      ipcSchemas[IpcChannel.CalibrationGetAvailability].response.parse({
        available: false,
        unavailableReason: 'hackersReason',
  ...
  it('CalibrationConflict kind enum rejects unknown kinds', () => {
    expect(() =>
      CalibrationConflict.parse({
        conflictId: OP_ID,
        profileId: PROFILE_ID,
        projectId: PROJECT_ID,
        kind: 'lastWriteWins', // Not valid
        entityId: PROJECT_ID,
        localPayloadSummary: null,
        serverPayloadSummary: null,
        serverRevision: 1,
...
// ==========================================================================
// Outbox idempotency / replay handling
// ==========================================================================
