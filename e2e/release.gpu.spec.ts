...
test(`@gpu packaged WebGL2 renders and interacts in ${gpuMode} mode`, async ({
  browserName,
}, testInfo) => {
  expect(browserName).toBe('chromium');
  test.setTimeout(120_000);
  const root = mkdtempSync(path.join(tmpdir(), `pf-gpu-${gpuMode}-`));
  ...
  try {
    ...
    await importModelFixture(launched.page, `packaged-gpu-${gpuMode}-fixtures`);
    ...
    if (
      ...
      await testInfo.attach('packaged-process.log', {
        body: Buffer.from(launched.processOutput()),
        contentType: 'text/plain',
      });
    }
    await cleanupPackagedApp(launched, [root]);
  }
});
