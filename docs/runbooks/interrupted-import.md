...
2. Read diagnostics on `calibration:getDiagnostics`. `outbox`
   ...
   whether that matters: `noProfileSelected` is benign — select a profile and
   read again; `readFailed` means the sidecar read threw and you should follow
   `failed-migration.md` before re-running the import. `notAttempted` should not
   appear from this command; if it does, report it rather than acting on it.
