...
import './CalibrationConflictDialog.css';
...
export function CalibrationConflictDialog({
  ...
  onClose,
  onResolved,
}: CalibrationConflictDialogProps): React.JSX.Element {
  const titleId = useId();
  ...
  return (
    <>
      <div className="cal-conflict-backdrop" aria-hidden="true" />
      <section
        ref={dialogRef}
        className="cal-conflict-dialog"
        role="dialog"
        aria-modal="true"
        ...
        tabIndex={-1}
      >
        <header className="cal-conflict-header">
          <div>
          ...
          <button
            type="button"
            className="cal-conflict-close"
            aria-label="Close calibration conflict dialog"
            onClick={onClose}
          >
            &times;
          </button>
        ...
        {resolutionReport ? (
          <section
            className="cal-conflict-resolution-report"
        ...
        <div className="cal-conflict-content">
          <section
            className="cal-conflict-list-pane"
            aria-label="Conflict list"
          >
            <div className="cal-conflict-list-header">
              <div>
            ...
            {loadError ? (
              ...
              <div className="cal-conflict-state">
              ...
              >
                {conflicts.map((conflict, index) => (
                  <li
                    ...
                    className={`cal-conflict-option${conflict.conflictId === selectedId ? ' selected' : ''}`}
                    role="option"
                  ...
                  >
                    ...
                    <span className="cal-conflict-status-text">
                      Status: unresolved
                    </span>
                  </li>
                ))}
          ...
          <section
            className="cal-conflict-detail-pane"
...
function ConflictDetail({
  ...
  return (
    <article className="cal-conflict-detail">
      <header>
        <h3>{kindLabel(conflict.kind)} conflict</h3>
        <p>Entity {conflict.entityId}</p>
      </header>

      <div className="cal-conflict-comparison">
        <section>
      ...
      <form
        className="cal-conflict-resolution"
        onSubmit={(event) => {
          event.preventDefault();
      ...
      >
        <fieldset disabled={pending}>
          ...
          {conflict.availableResolutions.map((available) => (
            <label className="cal-conflict-choice" key={available}>
              <input
                type="radio"
        ...
        {manual ? (
          <div className="cal-conflict-manual">
