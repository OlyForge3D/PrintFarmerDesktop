...
- SQL injection (unsanitized user input in queries against the Rust/SQLite model-core)
- Command injection (user input in shell commands, Electron IPC handlers)
- Path traversal (user input in file paths without validation — especially model/library file access)
...
- Electron IPC channels exposed without contextIsolation/sandboxing review
...
3. **🔴 Red** — Work blocked. Reviewer Rejection Protocol activates:
   - The original author fixes their own work — the revision is not reassigned
   - Rai supplies the citations and counter-evidence the author needs to revise with grounding
