...
4. **Publish the extraction rule with the result.** State the pattern, the filter, and the head — and where the enumeration was run on a branch, name the **mainline** commit it was current with, because a set closed on a branch is closed only as of that branch's base. A shared-figure count is not reproducible without them, and a count that cannot be re-derived is the next defect rather than evidence.
