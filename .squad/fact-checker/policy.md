...
2. **Enumerate every rendering, not two — the question is which renderings _exist_, not which ones disagree.** Search the whole tree for the quantity or claim — `.squad/`, `docs/`, source comments, test doc comments, issue and PR bodies. A pair-wise habit is how a third rendering survives a repair that fixed the other two. The reason is structural and needs no account of anyone's behaviour: **a repair can only cover the renderings that existed when it was made.** Any rendering created afterwards is outside its scope, and so is any rendering the repairer did not enumerate. It follows that no diff restricted to the copies known at repair time can be complete, however symmetric. **Enumeration at the current head is the only thing that closes the set — and _the current head_ means the mainline's, not the checking branch's.** A branch that is behind is a stale enumeration by construction: renderings created on the mainline after that branch's base are outside every set the branch has measured, and the branch's own head reports nothing about their existence. This is recorded from measurement rather than supposed — see run H in `.squad/fact-checker/audit-trail.md`, where three new renderings of one quantity landed on the mainline while this check's own branch was behind, and an enumeration at the branch head would have returned the earlier set and reported it closed. **And the hits must be read, not counted.** A search cannot distinguish a **rendering** — the file asserting the value — from a **mention**, where it quotes another artifact. A retraction is the worst case, because it necessarily contains the sentence it withdraws: **a search for a refuted claim scores a hit on the document that refuted it.** Mechanical filters help and are worth running — see `scripts/measure-mention-filter.mjs`, which measures two of them — but **treat any filter as triage that narrows what must be read, never as a discriminator that licenses a count.** A filter can be checked for precision by printing what it removed; its **recall cannot be established without reading the hits it kept**, which is the work it was supposed to save. Run I records both candidates measured before either was adopted, and the result that one of them had zero adoption to describe.
3. **Establish precedence by ancestry, not by timestamps.** Two commits can be minutes apart and belong to branches that never met, so wall-clock order does not establish that one was available when the other was written. A claim of the form _"written after its correction was available"_ requires `git merge-base --is-ancestor <correction> <rendering>` to exit 0, **or** the correcting content to be present on the mainline at that time. **Check both, because a squash merge lands the content without the commit**: the correcting commit may appear in no ancestry while its text is plainly on the default branch, and the converse — a commit reachable nowhere near the mainline — is what makes proximity in time worthless. **Prefer the strongest available form: the correction present in the rendering's own parent tree.** That is a fact about the tree the commit was written against, and it requires no claim about what any author knew, saw, or should have noticed — claims which this procedure does not license and cannot support. **This rule exists because this file's first batch got it wrong** — see the corrected entry for run D in `.squad/fact-checker/audit-trail.md`. **And a reachability claim must name the space it searched.** `git branch --contains` searches **branches**; `refs/pull/N/head` and tags are not branches, so a zero from it means _not present among branches_ and never _absent_. **A positive control does not repair this.** A control that fires establishes the instrument is **sensitive within the space it searches** and is silent about the boundary of that space — so where both arms of the control are the same kind of ref, the control cannot detect that the question ranges wider than the search. **Reachability can be proved by a single containing ref and cannot be disproved by an incomplete enumeration of refs**, which makes _unreachable_ the harder claim and the one that needs its scope stated. Run J records this measured on a live blocker, where a correct measurement over branches carried a false conclusion about an object that `refs/pull/68/head` reaches. **And when a citation's target is branch-unreachable by construction — a pre-squash revision, which by definition never landed — give the reader the fetch command, because an instruction to verify somewhere unreachable is not a citation.**
4. **Publish the extraction rule with the result.** State the pattern, the filter, and the head — and where the enumeration was run on a branch, name the **mainline** commit it was current with, because a set closed on a branch is closed only as of that branch's base. A shared-figure count is not reproducible without them, and a count that cannot be re-derived is the next defect rather than evidence. **Read the base by ref name, not from a value quoted to you.** A SHA in a message is a measurement whose timestamp the reader cannot see, and it resolves, greps and answers every question put to it while being stale — so it fails silently in the one way a wrong pointer would not. `git ls-remote <url> refs/heads/<branch>` at the moment the enumeration begins. **A push report — _pushed X from Y_ — is exempt, because it records an event rather than the current value of a ref, and an event does not decay.** **And then publish the value you read, not the command that read it.** The exemption is not a permission, it is a **requirement**: a base pin is a closed fact about which tree the measurement ranged over, so suppressing it does not make the claim safer, it makes it **unreproducible** — strictly worse than a stale pin, because a stale pin is falsifiable and an absent one is not. **An over-broad rule against pins fails silently in the direction of _less_ evidence, which is the direction no reviewer can detect**: a reader who cannot reproduce a figure sees their own failure, not the omission that caused it. Run L records this file violating its own step 4 in the same commit that wrote the exemption, and a reviewer coming within one step of filing a blocker against a correct measurement because the scope had to be guessed.

   **Stating the head is a disclosure and not a check, so state it _and assert it live at publication_ — re-read from the remote at send, not at draft.** A head that was true when the work was done and false when the report shipped **satisfies every disclosure rule and misleads every reader**, which is why _"say which head"_ is not the repair it appears to be: **it has no falsifier, because you can always say which head.** _"This was live at send"_ can be wrong, and a rule that cannot be wrong is not a control. The gap is the **drafting interval** — the pin is taken while the work is done and the report ships later, and nothing between the two re-reads it. Recorded because four separate sessions have now shipped a stale-SHA warning **from a stale SHA**, each having stated the rule correctly in the same artifact: **the warning does not protect the party issuing it, and no amount of care at draft time can, because the defect is entirely in the interval afterwards.** Adopted from the coordinator, whose formulation is stronger than the one this file proposed.

5. **Run a control that can report non-empty.** A rule that returns nothing is not the same as a rule that finds agreement. **The same requirement binds any instrument that reports _identical_ or _absent_: it must be shown capable of reporting the other answer, over the space actually searched.** Agreement from an instrument that always agrees is not evidence, and neither is a zero from a search whose region is narrower than the question. **And a re-enumeration rule that has only ever produced changes is indistinguishable from one that always reports change** — its first clean run is what establishes it discriminates. See run K. **Run the pattern where the answer is known, in both directions.** A detector that fires in the passing case has no discriminating power, and a pattern that matches nowhere may be dead rather than describing an absence — both are constants wearing the costume of a measurement, and neither announces itself. **Absence must be tested for directly and never inferred**: not from two things comparing equal (two absences compare equal), not from an empty grep, not from a zero count, not from an empty ref listing — in each case _no result_ and _no such thing_ arrive as one value.
...
state that the object's absence is itself the finding. Checked by `scripts/check-citation-reachability.mjs` — run it with `npm run check:citation-reachability`. **Enforced on every pull request.** The workflow is live at `.github/workflows/citation-reachability.yml`: it was moved there in `e3a0e98`, armed in `4d1937a`, and the staged copy this document used to name was deleted in `299b33c`, so that path no longer exists. It could not be pushed from the authoring branch — that token lacks the `workflow` OAuth scope and the Contents API refused the same path, both attempted and measured — and a maintainer completed the move. The licence for the word is still held by a test rather than by a promise: `tests/citationReachability.test.ts` fails if any of these artifacts claims enforcement while no live workflow invokes the harness, **and now also fails if any of them denies enforcement while one does**. The second direction was added only after all three of these documents sat stale for hours behind a guard that looked one way — a harness which fails on any cited revision that is neither
...
**A property is not stale only through the passage of time; it is stale the moment another writer can change it.** Time is the assumed mechanism and it is not the only one. This batch verified `git rev-list --merges` empty immediately before a push, reported **no merge commits**, and was correct — and within the hour a coordinator ran the REST `update-branch` to keep an approving review reachable, which creates a **merge commit**. The action was deliberate and correct; the property was gone anyway. **So a claim about branch shape is an event claim and must be reported as one:** _"no merge commits at `<sha>`, verified at push"_ stays true forever, while _"the branch has no merge commits"_ can be falsified by someone who is not the author, without the author pushing anything, and with nothing on the author's side to warn them. **The instrument closest to hand is the one that cannot see it** — the local worktree stayed clean and correct and knew nothing. **Re-read shape from the remote ref at the moment the claim is used, never from the local checkout.** See run M.
...
- **Never cite a line number into a file that is not yet on `development`.** A line number is invalidated by any edit above it, and an unmerged file — one still open in a PR, still being rebased or squashed — moves. Worse, it keeps resolving: the file still exists, the line still holds content, so the citation never fails, it just silently starts pointing at different prose. `.squad/decisions.md` → **2026-08-09 — A line-number citation into an unmerged file keeps resolving and points somewhere else**. Cited by heading, not line number, for exactly this reason.
  ...
  - For a file **already on `development`**, a line number is _usable_, but this does not relax the older rule at `.squad/decisions.md` → **2026-07-25 — Review lessons from PRs #68 and #69** ("Grep for the expression, not the line"): a merged file keeps moving too, so a bare line number decays on the next unrelated edit above it exactly as it does pre-merge, only slower. Prefer a greppable expression or heading over a bare line number whenever the citation needs to survive future edits — a security precondition, a cited rule, anything load-bearing. A line number is acceptable shorthand only for a citation checked immediately against the current head, not one meant to outlive it.
  - **An exact quotation does not validate the line-number pointer beside it.** Quote and location are independent claims; check both. A correct quote makes a wrong or stale line number _harder_ to notice, not easier — the reader lands on real prose and has no signal they are in the wrong place.
