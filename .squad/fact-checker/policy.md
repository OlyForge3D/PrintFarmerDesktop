...
2. **Enumerate every rendering, not two — the question is which renderings _exist_, not which ones disagree.** Search the whole tree for the quantity or claim — `.squad/`, `docs/`, source comments, test doc comments, issue and PR bodies. A pair-wise habit is how a third rendering survives a repair that fixed the other two. **Corrections do not propagate to renderings that do not yet exist**: a repair fixes the copies visible at the time, and the next author reaches for whichever copy is nearest, which may be a superseded one. So a figure can be re-rendered into a _new_ document while a correction exists somewhere in the repository, and no diff between the copies known at repair time will ever see it. Enumeration at the current head is what finds those, and it is what found the live one in this file's first batch.
3. **Establish precedence by ancestry, not by timestamps. A commit is not "on the record" for an author whose commit does not descend from it.** Two commits can be minutes apart and belong to branches that never met; the later one's author may have had no ordinary means of seeing the earlier. So a claim of the form _"written N minutes after its correction was published"_ requires `git merge-base --is-ancestor <correction> <rendering>` to exit 0, or the correction to be reachable from the mainline at the time the rendering was authored. **Check both, because a squash merge lands the content without the commit**: the correcting commit may never appear in any ancestry while its text is plainly present on the default branch, and the reverse — a commit reachable nowhere near the mainline — is what makes wall-clock proximity worthless. Where the correction is _in the rendering's own parent tree_, say that instead: it needs no assumption about what the author could see. **This rule exists because this file's first batch got it wrong** — see the corrected entry for run D in `.squad/fact-checker/audit-trail.md`, where a 45-minute gap to a commit on a divergent branch was reported as a missed correction.
...
**And be clear about what ✅ then asserts, because it is not what the diff was for.** Once the value has been derived from the object, the independence question **has no work left to do**. The grade exists because agreement between renderings is only evidence when the renderings are independent; with ground truth in hand you no longer reason from their agreement at all — you check each rendering against ground truth, one at a time. **The diff is superseded, not validated.** So ✅ is a statement about **conformance of every enumerated rendering to a named non-rendering artifact**, and nothing about authorship, and nothing about whether the renderings agreed. It follows that source recovery is not the exemplar of a well-graded diff: it is the thing that makes the diff's own grade moot. Reaching it is the good outcome; claiming the diff earned it is not.
