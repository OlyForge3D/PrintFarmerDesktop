...
The rule then paid for itself in the other direction, twice over. I flagged on #78 at 18:41 that `threemf.rs` was modified by both it and #69 — 77 minutes before #69 landed. After the merge I re-ran the measurement rather than assuming the flag still described the situation, and the known overlap had hardened into a real conflict: `git merge-tree --write-tree` returns `CONFLICT (content)` on that file. So the early measurement caught the risk and the late one caught its change of state. **Confirming a known overlap is worth as much as discovering one** — an overlap that was harmless at one head can be a hard conflict at the next, and only re-measuring tells you which you have.
...
The common shape is not carelessness about facts. Every one of the three was checked, and every one was checked against a snapshot that had already expired. **A present-perfect negative — "has never been," "does not touch," "is not a consumer of" — about an open branch is a claim that ages badly by construction**, and a decision log is exactly where such a claim goes to be read months later by someone who will not re-derive it. So: **pin claims about moving objects to the SHA they were true of**, in this file as strictly as on a PR review. If a sentence cannot name the commit it holds at, it does not belong here.
