...
That the error **recurred across four rounds and was caught on every one**, and was then reintroduced _by the fix for it_, is the strongest argument in this file for external review. Nothing survived the check — each instance was rejected on the pass it appeared, including the round whose entire subject was making claims that do not expire. A fifth pass then found two further instances in paragraphs written days earlier and untouched since, using the rule this entry had just added — one of them in a citation that had itself been cleared across three rounds of an earlier review. That is the part worth generalising: **the newest text is where a defect is most likely, but not where it is most costly.** Four rounds of weighting new text hardest caught four defects before they shipped; one pass applying the rule to old text caught the two that already had.
