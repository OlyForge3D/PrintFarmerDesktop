...
That the error survived three rounds of fact-check and was then reintroduced _by the fix for it_ is the strongest argument in this file for external review. Each round the defect was in the text that round had just added — including the round whose entire subject was making claims that do not expire.
...
So: semantic coupling is a real hazard in principle, and **as of `085d91a` this repo contained no instance of it**. When one occurs, record it then — pinned, per the rule above, which applies to this sentence as much as to the ones fact-check caught. A decision log that carries a rule nobody can reproduce from the cited evidence is worse than one that stays silent, because the unreproducible rule is what future sessions cite.
