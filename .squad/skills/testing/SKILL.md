...
**Do not clear an npm-tree or downstream SBOM completeness failure by re-running it.** On Windows, `npm ci` can report an `EPERM` cleanup failure as a warning and exit 0 while leaving a partial `node_modules` tree. That is an environment/install failure, not evidence that the supply-chain policy is flaky. Preserve the first-attempt log and investigate the install. Every workflow now runs `scripts/ci-install.mjs`, which must stop a damaged tree at `Install dependencies`; if the same symptom reaches an SBOM step again, the install guard itself has failed and the run must not be dismissed.
