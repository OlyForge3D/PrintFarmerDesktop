...
2. **Non-empty ground truth.** Assert the _code-derived_ list is non-empty before comparing. Symmetry defeats one side going empty; it does not defeat both. If a refactor breaks the code-side extractor — renamed type, changed export shape, regex drift — empty equals empty and the test is green while guarding nothing.
