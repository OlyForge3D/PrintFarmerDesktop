...
**Re-measuring is necessary and not sufficient, and this note has a first-hand case.** Seconds after a successful push, `gh pr view 163 --json headRefOid` returned the **previous** head. That was a fresh read of a cached view — a correct measurement of the wrong thing, which is the same defect one layer down from the one being guarded against. The remedy for a mutable source is therefore not one fresh read but **agreement between two sources with different caches**: `gh` against `git rev-parse origin/<branch>` after an explicit fetch, which is how this instance was caught rather than published. **A single read cannot detect its own staleness**, and the more authoritative the source, the more confidently a stale answer from it gets repeated.
