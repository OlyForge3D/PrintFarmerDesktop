...
This is the same failure shape as a commit-pinned URL serving a superseded document (`.squad/decisions/inbox/ripley-go-and-look.md`), except the pointer here is _more_ fragile: a line number is invalidated by any edit above it, not only by a rewrite of the whole object.

**The instance.** `.squad/decisions/inbox/ripley-go-and-look.md` was, at the time of citation, in open PR #163 and not on `development`. It was cited as `ripley-go-and-look.md:37` for the rule _"never reconstruct an identifier — copy it from the tool that emitted it."_ Read at the object, on PR #163's head:
