...
independently _again_ in that batch — a second rediscovery of the exact
...
the alarming one: `""` conclusion → _passed_ (4), any ancestor →
_the tip_ (5), rewritten-but-not-mine → _fine_ (6, 8), `BEHIND` → _healthy_
(10), empty `reviewDecision` → _not blocked_ (11), stale exit code →
_success_ (9). The direction is not random — the reassuring reading is the
...
- `.squad/decisions/inbox/ripley-516-absence-not-removal.md` — a checker
  that reports a symbol, path, or reference as "missing" by reading only
  head merges `{removed, never existed}` into one cell. It is the base/head
